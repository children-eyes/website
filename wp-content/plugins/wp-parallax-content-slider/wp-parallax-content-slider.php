	...
	public function get_parallax_content_slider( $category = '' )
	{
		...
		while ( $myposts->have_posts() ) : $myposts->the_post();
			...
			if ($prlx_text_content === 'excerpt') {
				$outputDynamic .= $this->get_article_slide($post, $prlx_title_max_chars);
			} else {
				$outputDynamic .= $this->get_article_slide($post, $prlx_title_max_chars);
			}
...
DYNAMICOUTPUT;*/
	 ...
	 */
	function get_article_slide($post, $prlx_title_max_chars)
	{
		get_template_part( 'slider-children');
	}
	...
	function admin_interface()
	{
		if ( isset($_POST['action']) && ($_POST['action'] == 'update') )
		{
			...
			if ( $validation )
			{
				...
				if ( isset($_POST['prlx_slider_category_filter']) && !empty( $_POST['prlx_slider_categories'] ) ) {
				...
				$prlx_slider_settings = array(
						...
						'category_filter' 		=> isset($_POST['prlx_slider_category_filter']) ? $_POST['prlx_slider_category_filter'] : '',
						'categories'			=> isset($categories_selected_values) ? $categories_selected_values : '',
					...
					<tr>
						...
						<td>
							<select name="prlx_slider_content_type" id="prlx_slider_content_type">
								...
								<option value="children" <?php if ( $prlx_slider_settings['content_type'] === "children") echo 'selected="selected"'; ?>><?php _e( 'Children-Eyes', 'wp-parallax-content-slider' ); ?></option>
